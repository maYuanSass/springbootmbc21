...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','tushufenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','tushufenlei');
			...
			async init(){
				...
				let res = await this.$api.info('tushufenlei', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusstushufenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
